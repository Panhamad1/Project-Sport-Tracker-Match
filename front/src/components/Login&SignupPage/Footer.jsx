...
import { FaGlassCheers, FaFutbol, FaTrophy, FaEnvelope, FaPhone, FaMapMarkerAlt } from 'react-icons/fa';
...
const Footer = () => {
    ...
    const quickLinks = [
        ...
        { path: '/leagues', label: 'Leagues', public: true },
        { path: '/matches', label: 'Matches', public: true },
        { path: '/standings', label: 'Standings', public: true },
        { path: '/teams', label: 'Teams', public: true },
        { path: '/players', label: 'Players', public: true },
        { path: '/compare', label: 'Compare', public: true },
        { path: '/news', label: 'News', public: true },
        { path: '/leaderboard', label: 'Leaderboard', public: false },
        { path: '/prediction', label: 'Predictions', public: false },
        { path: '/profile', label: 'Profile', public: false },
    ];

    const visibleLinks = quickLinks.filter(link => link.public || isAuthenticated);
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="grid grid-cols-[repeat(auto-fit,minmax(200px,1fr))] gap-6 md:gap-8">
                        <div className="space-y-3">
                            ...
                            <p className="text-gray-400 text-sm leading-relaxed">Your ultimate destination for live football scores and stats.</p>
                        ...
                        <div>
                            ...
                            <ul className="space-y-2">
                                {visibleLinks.map((link) => (
                                    <li key={link.path}>
                                        <Link to={link.path} className="text-gray-400 hover:text-[#8b5cf6] transition-colors text-sm hover:translate-x-1 inline-block duration-200">{link.label}</Link>
                                    </li>
                                ))}
                            </ul>
                        </div>
                        ...
                        <div>
                            <h4 className="text-white font-semibold mb-3 text-sm uppercase tracking-wider">Top Leagues</h4>
                            <ul className="space-y-2">
                                {leagues.map((league) => (
                                    <li key={league}>
                                        <Link to={`/league/${league.toLowerCase().replace(/ /g, '-')}`} className="text-gray-400 hover:text-[#8b5cf6] transition-colors text-sm hover:translate-x-1 inline-block duration-200">{league}</Link>
                                    </li>
                                ))}
                            </ul>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="mt-4">
                                ...
                                <div className="flex flex-col sm:flex-row gap-2">
                                    <input type="email" placeholder="Your email" className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg px-3 py-1.5 text-white text-sm placeholder-gray-500 focus:outline-none focus:border-[#8b5cf6] transition-colors flex-1 min-w-30" />
                                    <button className="bg-linear-to-r from-[#8b5cf6] to-[#3b82f6] text-white text-sm font-medium px-4 py-1.5 rounded-lg hover:shadow-lg hover:shadow-purple-500/20 transition-all hover:scale-[1.02] active:scale-[0.98] whitespace-nowrap">Submit</button>
                                </div>
                            </div>
                        </div>
                    </div>
